...
class TestSqliteKnowledgeService(unittest.TestCase):
    ...
    def test_create_kb_and_list(self):
        kb = self._svc.create_kb({"name": "kb1", "description": "d"})
        self.assertEqual(kb["id"], "kb-1")
    ...
    def test_upload_and_sync_chunks(self):
        kb = self._svc.create_kb({"name": "kb1"})
        info = self._svc.save_upload(kb["id"], "a.pdf", None)
        self.assertEqual(info["status"], "uploaded")
        fid = int(info["id"].split("-")[1])
        ...
        self.assertEqual(chunks[0]["metadata"], {"k": 1})

        files = self._svc.list_files("kb-1")
        self.assertEqual(len(files), 1)
        self.assertEqual(files[0]["chunkCount"], 2)
