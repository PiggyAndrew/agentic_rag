...
from backend.api.models import KnowledgeBase, KnowledgeBaseCreate, KnowledgeBaseUpdate, KBDocumentCreate, IngestRequest, ApiResponse
from backend.api.mappers.kb import kb_document_to_api, kb_meta_to_api
...
def list_documents(kb_id: str, kb: KnowledgeBaseUseCase = Depends(get_kb_usecase)):
    """列出知识库下的文档。"""
    documents = kb.list_documents(kb_id)
    data = [kb_document_to_api(document) for document in documents]
    return ApiResponse(ok=True, data=data)


@router.post("/api/kb/{kb_id}/files", response_model=ApiResponse)
def upload_document(kb_id: str, payload: KBDocumentCreate, kb: KnowledgeBaseUseCase = Depends(get_kb_usecase)):
    """上传文档文件（可选 Base64），入库为未向量化状态。"""
    ...
    info = kb.save_uploaded_document(kb_id, name, payload.contentBase64)
    return ApiResponse(
        ok=True,
        data=kb_document_to_api(info),
    )


@router.get("/api/kb/{kb_id}/files/{document_id}/chunks", response_model=ApiResponse)
def read_document_chunks(kb_id: str, document_id: str, kb: KnowledgeBaseUseCase = Depends(get_kb_usecase)):
    """读取指定文档的全部片段内容。"""
    data = kb.read_document_chunks(kb_id, document_id)
    return ApiResponse(ok=True, data=data)


@router.post("/api/kb/{kb_id}/ingest", response_model=ApiResponse)
def ingest_uploaded_document(kb_id: str, payload: IngestRequest, kb: KnowledgeBaseUseCase = Depends(get_kb_usecase)):
    """向量化处理已上传文档（PDF/Excel）。"""
    name = (payload.filename or "").strip()
    if not name:
        raise HTTPException(status_code=400, detail="filename 不能为空")
    info = kb.ingest_uploaded_document(kb_id, name)
    return ApiResponse(
        ok=True,
        data=kb_document_to_api(info),
    )
...
@router.delete("/api/files/{document_id}", response_model=ApiResponse)
def delete_document(document_id: str, kb: KnowledgeBaseUseCase = Depends(get_kb_usecase)):
    """删除文档（在所有知识库中查找并删除）。"""
    kb.delete_document_global(document_id)
    return ApiResponse(ok=True, data={"ok": True})
